...
If a tie occurs and you have bet on the player or the banker, you get your original bet back.
...
| Outcome     |   Occurrences |   Percentage |
|:------------|--------------:|-------------:|
...
| Outcome     | Bet on Banker | Bet on Player | Bet on Tie |
|:------------|--------------:|--------------:|-----------:|
| Banker Wins |          1.95 |             0 |          0 |
| Player Wins |             0 |          2.00 |          0 |
| Tie         |          1.00 |          1.00 |       9.00 |

The column labeled "Bet on Banker" indicates the money you receive in each scenario if you bet on the banker winning. This includes your original \$1 wager. If we win, we get back our original \$1 bet plus a \$0.95 payout since it pays 19-to-20. If the player wins, we lose our bet and get nothing. In a tie, we get back our original bet of \$1. We have similar columns for the money returned if we bet on the player or on a tie.

We multiply the returns in each column by the probabilities of each outcome and sum to get the expected return. We express this as a percentage of our original \$1 bet, so that it will scale to any dollar amount that we want to bet. The "house advantage" is then 1 minus this percentage.
...
Since Baccarat is played from a 6- or 8-deck shoe, you might naturally think there is an opportunity to count cards. The article below provides a great summary of several experts' opinions on this subject, each of them indicating that card counting is not practical in this game:
...
Ed Thorp does provide a card-counting strategy that can yield an advantage to bets made on the player in rare scenarios. A positive advantage of 0.329% occurs every 1,786 hands, which is far too rare to use to your advantage. John May (a baccarat author) developed a counting strategy for betting on a tie that he claims yields a 62% advantage in certain rare scenarios. However, these rare scenarios occur approximately once every 10,000 hands. Once again, this is too rare to be useful. If a player played Baccarat 40 hours a week, this scenario would occur only once every three weeks.
